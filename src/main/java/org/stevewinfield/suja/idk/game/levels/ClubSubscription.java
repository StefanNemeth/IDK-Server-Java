...
public class ClubSubscription {
    ...
    public int getAvailableGifts() {
        return this.hadMembership() ? (((int) Math.ceil(((int) (this.getPastClubTime() / 86400) + (int) (this
        .getPastVipTime() / 86400)) / 31)) - choosenGifts + 1) : 0;
    }

    public int getNextGiftSpan() {
        return this.hadMembership() ? 31 - (((int) (this.getPastClubTime() / 86400) + (int) (this.getPastVipTime() / 86400)) % 31)
        : 0;
    }
    ...
    public void expire() {
        ...
        "UPDATE player_subscriptions SET past_time_hc=" + hcTime + ", past_time_vip=" + vipTime
        + ", subscription_level=0, timestamp_created=0, timestamp_expire=0 WHERE player_id=" + this.playerId);
    }
    ...
    public void chooseClubGift(final CatalogClubGift gift, final Session session) {
        Bootloader.getStorage().executeQuery(
        "UPDATE player_subscriptions SET choosen_gifts=" + ++this.choosenGifts + "  WHERE player_id=" + this.playerId);
    ...
    public void update(final int level, final int time) {
        ...
        final boolean saveClubTime = this.baseLevel == ClubSubscriptionLevel.BASIC
        && level > ClubSubscriptionLevel.BASIC;
        ...
        if (Bootloader.getStorage().entryExists(
        "SELECT player_id FROM player_subscriptions WHERE player_id=" + this.playerId)) {
            Bootloader.getStorage().executeQuery(
            "UPDATE player_subscriptions SET subscription_level=" + level + ", timestamp_expire=" + expire
            + (saveClubTime ? ", past_time_hc=" + (int) this.hcTime : "") + " WHERE player_id=" + this.playerId);
        } else {
            Bootloader.getStorage().executeQuery(
            "INSERT INTO player_subscriptions (player_id, subscription_level, timestamp_created, timestamp_expire) VALUES ("
            + this.playerId + ", " + this.baseLevel + ", '" + this.created + "', '" + this.expire + "')");
        }
    }
