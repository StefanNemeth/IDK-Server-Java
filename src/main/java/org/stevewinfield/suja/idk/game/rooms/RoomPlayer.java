...
import org.stevewinfield.suja.idk.communication.room.writers.RoomPlayerWaveWriter;
...
public class RoomPlayer implements ISerialize {
    ...
    public boolean moveTo(Vector2 pos, final boolean ignoreKick, final boolean ignoreBlocking, final boolean isRollerEvent) {
        ...
        for (final RoomItem item : room.getRoomItemsForTile(pos)) {
            if ((topItem == null || item.getAbsoluteHeight() > topItem.getAbsoluteHeight())
            && item.getBase().isLayable())
                topItem = item;
        }
        ...
        if (Pathfinder.getPathInArray(room.getGamemap().getTileStates(), room.getGamemap().getTileHeights(), this
        .getPosition().getVector2(), pos, this.walkingBlocked) == null)
            return false;
    ...
    public void onCycle() {
        if (this.stepIsSetted()) {
            if ((this.room.getInformation().getModel().getDoorPosition().getX() == this.getSettedPosition().getX() && this.room
            .getInformation().getModel().getDoorPosition().getY() == this.getSettedPosition().getY())
            ...
            if (highestItem == null || (highestItem != null && this.floorItem != highestItem)) {
                if (this.floorItem != null) {
                    this.floorItem.onPlayerWalksOff(this,
                    floorItem.getBase().getInteractor() != FurnitureInteractor.WF_PLATE
                    && floorItem.getBase().getInteractor() != FurnitureInteractor.BATTLE_BANZAI_PUCK);
                    this.floorItem = null;
                }
                if (highestItem != null) {
                    this.floorItem = highestItem;
                    highestItem.onPlayerWalksOn(this,
                    highestItem.getBase().getInteractor() != FurnitureInteractor.WF_PLATE
                    && highestItem.getBase().getInteractor() != FurnitureInteractor.BATTLE_BANZAI_PUCK);
                }
            }
        }
        ...
        if (this.isWalking() && !this.isFrozen()) {
            ...
            if (!this.isRollerEvent) {
                Node l = (this.getPosition().getX() == this.getGoalPosition().getX() && this.getPosition().getY() == this
                .getGoalPosition().getY()) ? null : Pathfinder.getPathInArray(room.getGamemap().getTileStates(), room
                .getGamemap().getTileHeights(), this.getPosition().getVector2(), this.getGoalPosition(),
                this.walkingBlocked);
            ...
            if (point.getX() == this.getPosition().getX() && point.getY() == this.getPosition().getY() || updateMap) {
                ...
                if (this.getPosition().getX() == this.getGoalPosition().getX()
                && this.getPosition().getY() == this.getGoalPosition().getY()) {
                    ...
                    if (this.getActingItem() != null) {
                        this.getActingItem().getInteractor()
                        .onTrigger(this, this.getActingItem(), 0, this.room.hasRights(this.getSession()));
                ...
                } else if ((state = room.getGamemap()
                .getTileState(this.getPosition().getX(), this.getPosition().getY())) == TileState.LAYABLE
                    ...
                    if (highestItem != null) {
                        if (highestItem.getRotation() == 2) {
                            if (this.getPosition().getX() != highestItem.getPosition().getX()) {
                                this.setPosition(new Vector3(highestItem.getPosition().getX(), this.getPosition()
                                .getY(), this.getPosition().getAltitude()));
                            }
                        } else if (highestItem.getRotation() == 0) {
                            if (this.getPosition().getY() != highestItem.getPosition().getY()) {
                                this.setPosition(new Vector3(this.getPosition().getX(), highestItem.getPosition()
                                .getY(), this.getPosition().getAltitude()));
                            }
                        }
                        this.setRotation(highestItem.getRotation());
            ...
            if (updateMap) {
                ...
                if (highestItem == null || (highestItem != null && this.floorItem != highestItem)) {
                    if (this.floorItem != null) {
                        ...
                        if (floorItem.getBase().getInteractor() == FurnitureInteractor.WF_PLATE
                        || floorItem.getBase().getInteractor() == FurnitureInteractor.BATTLE_BANZAI_PUCK)
                            floorItem.getInteractor().onPlayerWalksOff(this, floorItem);
                    }
                    if (highestItem != null) {
                        if (highestItem.getBase().getEffectId() > 0) {
                            if (this.effectId == RoomPlayerEffect.BANZAI_BLUE
                            || this.effectId == RoomPlayerEffect.BANZAI_GREEN
                            || this.effectId == RoomPlayerEffect.BANZAI_ORANGE
                            || this.effectId == RoomPlayerEffect.BANZAI_PINK)
                        ...
                        if (highestItem.getBase().getInteractor() == FurnitureInteractor.WF_PLATE) {
                            ...
                            if (this.trigger) {
                                highestItem.getInteractor().onTrigger(this, highestItem, 1,
                                this.getSession() != null && room.hasRights(this.getSession()));
                                // this.trigger = false;
                            } else {
                                highestItem.getInteractor().onPlayerWalksOn(this, highestItem);
                            }
                ...
                if (!this.isRollerEvent) {
                    ...
                    this.setRotation(Rotation.calculate(this.getPosition().getX(), this.getPosition().getY(), nextX,
                    nextY));
                    this.setStepSetted(true);
                ...
                .updateTile(
                point,
                this.getRoom().getInformation().blockingDisabled() ? (TileState.PLAYER | (oxi ? this.currentTileState
                : 0)) : (TileState.BLOCKED | (oxi ? this.currentTileState : 0)));
            }
            this.update();
        }
        if (this.isBot()) {
            if (this.botInstance.isMovingEnabled() && this.botMoveCycles-- == 0) {
                ...
                for (final Vector2 vector : Pathfinder.DIRECTIONS) {
                    int state = 0;
                    if (((state = this.getRoom().getGamemap()
                    .getTileState(position.getX() + vector.getX(), position.getY() + vector.getY())) == TileState.OPEN || state == TileState.SITABLE)
                    && (room.getInformation().getModel().getDoorPosition().getX() != (position.getX() + vector.getX()) || room
                    .getInformation().getModel().getDoorPosition().getY() != position.getY() + vector.getY())) {
            ...
            if (this.botInstance.getPhrases().size() > 0 && this.botPhraseCylces-- == 0) {
                final BotPhrase phrase = this.botInstance.getPhrases().get(
                new Random().nextInt(this.botInstance.getPhrases().size()));
