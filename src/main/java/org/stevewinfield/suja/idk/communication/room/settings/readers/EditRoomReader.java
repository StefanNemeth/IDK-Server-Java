...
public class EditRoomReader implements IMessageReader {
    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        if (accessType != RoomAccessType.OPEN && accessType != RoomAccessType.BELL
        ...
        try {
            final PreparedStatement std = Bootloader.getStorage().queryParams(
            "UPDATE rooms SET name=?, description=?, access_type=" + accessType + ", password=?, max_players="
            + roomUserLimit + ", category_id=" + categoryId + ", allow_pets=" + (allowPets ? 1 : 0)
            + ", allow_pets_eating=" + (allowPetsEating ? 1 : 0) + ", hide_walls=" + (hideWalls ? 1 : 0)
            + ", wall_thickness=" + wallThickness + ", floor_thickness=" + floorThickness + ", disable_blocking="
            + (disableBlocking ? 1 : 0) + ", tags=? WHERE id=" + room.getInformation().getId());
        ...
        final Session owner = room.getInformation().getOwnerId() == session.getPlayerInstance().getInformation().getId() ? session
        ...
        if (owner != null) {
            for (final RoomInformation info : owner.getPlayerInstance().getRooms()) {
                if (info.getId() == room.getInformation().getId()) {
                    info.set(roomTitle, roomDescription, tags, accessType, roomPassword, roomUserLimit, categoryId,
                    allowPets, allowPetsEating, hideWalls, wallThickness, floorThickness, disableBlocking,
                    tradingEnabled);
                    break;
                }
            }
        ...
        categoryId, allowPets, allowPetsEating, hideWalls, wallThickness, floorThickness, disableBlocking,
        tradingEnabled);
        ...
        && Bootloader.getGame().getNavigatorListManager().getNavigatorLists().containsKey(oldCategory)) {
            Bootloader.getGame().getNavigatorListManager().getNavigatorLists().get(oldCategory)
            .removeRoom(room.getInformation());
        }
        ...
        if (disableBlocking != oldDisableBlocking) {
            for (final RoomPlayer player : room.getRoomPlayers().values()) {
                if (player.getCurrentTileState() > -1) {
                    final boolean oxi = player.getCurrentTileState() == TileState.SITABLE
                    || player.getCurrentTileState() == TileState.LAYABLE;
                    room.getGamemap().updateTile(
                    player.getPosition().getVector2(),
                    disableBlocking ? (TileState.PLAYER | (oxi ? player.getCurrentTileState() : 0))
