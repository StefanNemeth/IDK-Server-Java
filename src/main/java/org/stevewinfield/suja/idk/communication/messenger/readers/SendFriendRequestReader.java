...
public class SendFriendRequestReader implements IMessageReader {
    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        try {
            final PreparedStatement sthId = Bootloader.getStorage().queryParams(
            ...
            if (requestId < 1
            || requestId == session.getPlayerInstance().getInformation().getId()
            || session.getPlayerMessenger().getBuddies().containsKey(requestId)
            || session.getPlayerMessenger().getRequests().containsKey(requestId)
            || Bootloader.getStorage().readInteger(
            "SELECT COUNT(id) FROM player_friends WHERE player_req_id="
            + session.getPlayerInstance().getInformation().getId() + " AND player_acc_id=" + requestId) > 0) {
                return;
            }

            final PreparedStatement sthCrt = Bootloader
            .getStorage()
            .queryParams(
            "INSERT INTO player_friends (player_req_id, player_acc_id, request_playername, request_playerfigure, is_request) VALUES ("
            + session.getPlayerInstance().getInformation().getId() + ", " + requestId + ", ?, ?, 1)");
            ...
            if (requestSession != null && requestSession.isAuthenticated()) {
                ...
                request.set(session.getPlayerInstance().getInformation().getId(), session.getPlayerInstance()
                .getInformation().getPlayerName(), session.getPlayerInstance().getInformation().getAvatar());
                requestSession.getPlayerMessenger().getRequests()
                .put(session.getPlayerInstance().getInformation().getId(), request);
                requestSession.writeMessage(new MessengerRequestNoficiationWriter(session.getPlayerInstance()
                .getInformation().getId(), session.getPlayerInstance().getInformation().getPlayerName(), session
                .getPlayerInstance().getInformation().getAvatar()));
            }
