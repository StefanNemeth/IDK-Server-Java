...
public class RespectPlayerReader implements IMessageReader {
    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        room.writeMessage(new RoomPlayerRespectedWriter(targetSession.getPlayerInstance().getInformation().getId(),
        targetSession.getPlayerInstance().getInformation().getRespectPoints() + 1), session);
