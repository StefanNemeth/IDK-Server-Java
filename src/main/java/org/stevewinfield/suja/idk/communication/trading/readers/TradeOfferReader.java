...
public class TradeOfferReader implements IMessageReader {
    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        || !session.getPlayerInstance().getInformation().canTrade())
            return;
        ...
        if (item == null || !item.getBase().isTradeable()
        || !trade.offerItem(session.getPlayerInstance().getInformation().getId(), item))
            return;
