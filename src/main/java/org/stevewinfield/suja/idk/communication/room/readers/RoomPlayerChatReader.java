...
public class RoomPlayerChatReader implements IMessageReader {
    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        if (message.startsWith(":") && message.length() > 1) {
            ...
            && session.getPlayerInstance().hasRight(
            Bootloader.getGame().getChatCommandHandler().getCommand(command).getPermissionCode())
            && Bootloader.getGame().getChatCommandHandler().getCommand(command)
            .execute(session.getRoomPlayer(), new ChatCommandArguments(args, shouting))) {
                return;
            }
        }
