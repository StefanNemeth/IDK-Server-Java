...
public class RoomApplyDecorationReader implements IMessageReader {
    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        case FurnitureInteractor.WALLPAPER:
            key = "wallpaper";
            updateData = "wallpaper=" + item.getFlags() + ";floor="
            + room.getInformation().getDecorations().get("floor") + ";landscape="
            + room.getInformation().getDecorations().get("landscape");
            break;
        case FurnitureInteractor.FLOOR:
            key = "floor";
            updateData = "wallpaper=" + room.getInformation().getDecorations().get("wallpaper") + ";floor="
            + item.getFlags() + ";landscape=" + room.getInformation().getDecorations().get("landscape");
            break;
        case FurnitureInteractor.LANDSCAPE:
            key = "landscape";
            updateData = "wallpaper=" + room.getInformation().getDecorations().get("wallpaper") + ";floor="
            + room.getInformation().getDecorations().get("floor") + ";landscape=" + item.getFlags();
            break;
        }

        room.getInformation().setDecoration(key, item.getFlags());
        room.writeMessage(new RoomDecorationWriter(new AbstractMap.SimpleEntry<String, String>(key, item.getFlags())),
        session);
        Bootloader.getStorage().executeQuery(
        "UPDATE rooms SET decorations='" + updateData + "' WHERE id=" + room.getInformation().getId());
