...
public class CreateRoomReader implements IMessageReader {
    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        || (session.getPlayerInstance().getRooms().size() >= IDK.NAV_MAX_ROOMS_PER_PLAYER && !session
        .getPlayerInstance().hasRight("unlimited_rooms")))
            return;

        final String roomName = InputFilter.filterString(reader.readUTF());
        final String modelName = reader.readUTF().toLowerCase();

        if (roomName.length() < 3 || roomName.length() >= 26 || modelName.length() >= 10
        || Bootloader.getGame().getRoomManager().getRoomModel(modelName) == null)
            return;

        int id = 0;
        try {
            final PreparedStatement std = Bootloader.getStorage().queryParams(
            "INSERT INTO rooms (owner_id, name, description, model_name) VALUES ("
            ...
            final ResultSet row = Bootloader.getStorage().queryParams("SELECT * FROM rooms WHERE id=" + id)
            .executeQuery();
