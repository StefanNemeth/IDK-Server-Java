...
public class AddFavoriteReader implements IMessageReader {
    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        || session.getPlayerInstance().getFavoriteRooms().size() >= IDK.NAV_MAX_FAVORITES)
            return;
        ...
        try {
            ...
            .getStorage()
            .queryParams(
            "INSERT INTO player_room_favorites (player_id, room_id) VALUES ("
            + session.getPlayerInstance().getInformation().getId() + ", " + roomId + ")").execute();
