...
public class RecycleItemsReader implements IMessageReader {
    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        final GapList<Integer> recyclerRewards = Bootloader.getGame().getCatalogManager().getCatalogRecyclerRewards()
        .get(level);
        final int randomId = random.nextInt(recyclerRewards.size());
        final Furniture recyclerReward = Bootloader.getGame().getFurnitureManager()
        .getFurniture(recyclerRewards.get(randomId));
        ...
        session.writeMessage(new CatalogRecyclerResultWriter(true, session
        .getPlayerInstance()
        .getInventory()
        .addItem(Bootloader.getGame().getFurnitureManager().getFurniture(IDK.CATA_RECYCLER_BOX_ID), session, 1,
        (new SimpleDateFormat(IDK.SYSTEM_DATE_FORMAT).format(new Date())) + (char) 10 + recyclerReward.getId(), null)));
