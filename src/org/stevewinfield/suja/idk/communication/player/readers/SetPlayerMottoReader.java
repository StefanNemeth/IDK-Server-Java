...
public class SetPlayerMottoReader implements IMessageReader {
    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        try {
            ...
            "UPDATE players SET motto=? WHERE id=" + session.getPlayerInstance().getInformation().getId());
        ...
        if (session.getPlayerInstance().getInformation().isStreamEnabled()) {
            session.getFriendStream().broadcastEvent(session, FriendStreamEventType.MOTTO_CHANGED,
            FriendStreamLinkType.OPEN_MOTTO_CHANGER, new String[] { motto });
        }
